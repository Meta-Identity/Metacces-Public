...
contract AccesVesting {
    ...
    function claimMonthlyAmount() external isInvestor(msg.sender) isNotBlackListed(msg.sender) nonReentrant{
        ...
        if(investor[msg.sender].yAllowed > investor[msg.sender].amount){
            investor[msg.sender].yAllowed = investor[msg.sender].amount;
            investor[msg.sender].mAllowed = investor[msg.sender].yAllowed.mul(_mP).div(hPercent);
        ...
        if(investorAllowance[msg.sender].endYear <= block.timestamp){
            ...
            investorAllowance[msg.sender].yearlyAllowed = leftOver.add(investor[msg.sender].yAllowed);
        }
        uint256 monthLock = investor[msg.sender].monthLock;
        uint256 yearlyAmount = investor[msg.sender].yAllowed;
